...
- **The docstring feeds the man page; never start a prose line with `-`.**
  `make man` runs `lsc --help` through help2man, which treats any line that
  begins with a dash as an option definition and reformats that paragraph,
  mangling it. Keep flag mentions mid-sentence (e.g. "Both --set and --rm ...")
  or reword; only the real Options block should have dash-led lines.
